...
public class LoginDataLayerImpl extends AbstractDataLayer implements LoginDataLayer {
	...
	public DigitalData login(final String error, final HttpServletRequest request, final HttpServletResponse response, 
		...
		return new DigitalDataImpl.Builder()
				.pageInstanceID("login-prod")
				.page(loginPageAdapter(request, device))
	...
	private Page loginPageAdapter(final HttpServletRequest request, final Device device) {
		...
		return new PageImpl.Builder()
			...
			.category(new CategoryImpl.Builder()
					.primaryCategory("login") // we use site section as the primary category for our demo retail app
				.build())
